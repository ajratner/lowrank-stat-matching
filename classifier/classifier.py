...
def get_speeding_stats(levels_up=1, n_other=10000, source_file=SOURCE_FILE_SPEEDING):
  ps = PorterStemmer()
  tfidf = TfidfVectorizer(stop_words=SW, ngram_range=(1,1), sublinear_tf=True)
  stats = []
  by_id = {}
  ...
  with DB_connection() as handle:
    ...
    if levels_up > 0:
      ...
      cids = random.sample(cids, max_other)
      for cid in cids:
...
FEATURES = [1,0,0,0,0]
def statute_pairs(stats, wes, inv_index, fs=FEATURES):
  X = []
  Y = []
  for i,s1 in enumerate(stats):
    for j,s2 in enumerate(stats):
      if j >= i:
        ...
        X.append(f)
        Y.append(1.0 if s1.label == s2.label and s1.state != s2.state else 0.0)
...
def train_classifier(X,Y):
  ...
  for c in C_range:
    ...
    for train_i, test_i in skf:
      ...
      scores.append(matthews_corrcoef([Y[i] for i in test_i], Y_est))
